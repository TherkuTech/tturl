      ...
      <form onSubmit={handleGenerate}>
        <div className="relative w-full max-w-xl">
          <input
            ...
            className={`w-full px-6 py-3 ${darkMode ? "text-white/70 bg-gray-800 placeholder-gray-400":  "text-black bg-neutral-400/45 placeholder-gray-500"} rounded-full outline-none focus:ring-2 focus:ring-blue-500 ${
        ...
        <div className="flex flex-col gap-3 mt-6">
          <div className={`${!darkMode ? "text-black/70":  "text-white/70"} `}>Your shortened URL:</div>
          <div
            className={`flex w-full h-12 rounded-md items-center px-4 ${darkMode ? "text-white/70 bg-gray-800":  "text-black bg-neutral-400/45"} ${
              copiedToClipboard
                ? "border border-green-500"
                : "border border-gray-700 dark:border-gray-400"
            }`}
          >
            <div className={`w-[240px] truncate ${!darkMode ? "text-black/70":  "text-white/70"} `}>
              {shortenedUrl ? (
                `${process.env.NEXT_PUBLIC_FRONTEND_URL}/${shortenedUrl}`
              ) : (
